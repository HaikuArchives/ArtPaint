...
VisualColorControl::VisualColorControl(
	BPoint position, rgb_color c, const char* s1, const char* s2, const char* s3, const char* s4)
	:
...
void
VisualColorControl::AttachedToWindow()
{
	...
	BStringView* sv
		= new BStringView(BRect(0, COLOR_HEIGHT / 2, 1, COLOR_HEIGHT / 2), "label view", label1);
	AddChild(sv);
...
void VisualColorControl::Draw(BRect)
{
	...
	// here draw the ramps
	BRect ramp_rect
		= BRect(ramp_left_edge + 4, 6, ramp_left_edge + RAMP_WIDTH + 4, 6 + ramp_height - 1);
	...
	DrawPicture(down_arrow,
		BPoint(ramp_left_edge
			+ (float)((value_at_1() - min_value_at_1()))
				/ (max_value_at_1() - min_value_at_1()) * RAMP_WIDTH + 4,
			5));
	DrawPicture(down_arrow,
		BPoint(ramp_left_edge
			+ (float)((value_at_2() - min_value_at_2()))
				/ (max_value_at_2() - min_value_at_2()) * RAMP_WIDTH + 4,
			COLOR_HEIGHT + 5));
	DrawPicture(down_arrow,
		BPoint(ramp_left_edge
			+ (float)((value_at_3() - min_value_at_3()))
				/ (max_value_at_3() - min_value_at_3()) * RAMP_WIDTH + 4,
			2 * COLOR_HEIGHT + 5));
	DrawPicture(down_arrow,
		BPoint(ramp_left_edge
			+ (float)((value_at_4() - min_value_at_4()))
				/ (max_value_at_4() - min_value_at_4()) * RAMP_WIDTH + 4,
			3 * COLOR_HEIGHT + 5));
	Sync();
...
void
VisualColorControl::MessageReceived(BMessage* message)
{
	rgb_color* new_color;
	ssize_t color_size;
	switch (message->what) {
...
void
VisualColorControl::SetValue(int32 val)
{
	value.word = val;
	CalcRamps();
	Draw(Bounds());
