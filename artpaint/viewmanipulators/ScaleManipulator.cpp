	...
	BBitmap *original, BStatusBar *status_bar)
{
	...
	if (new_width != starting_width) {
		...
		if (diff < 1) {
			...
			for (int32 y = 0; y < starting_height; y++) {
				accumulation = 0;
				uint32* src_bits = source_bits + (int32)original_left +
					(y + (int32)original_top) * source_bpr;
			...
			for (int32 y = 0; y <= starting_height - 1; y++) {
				uint32* src_bits = source_bits + (y + (int32)original_top) * source_bpr;
				float fx = original_left;
				for (int32 x = 0; x < target_bpr; x++) {
...
int32 ScaleManipulator::PreviewBitmap(bool, BRegion* region)
{
	...
	if (selection == NULL || selection->IsEmpty() == true) {
		...
		if (settings->left != original_left ||
			settings->top != original_top) {

			selection->Translate(settings->left - original_left,
				settings->top - original_top);
			selection->Recalculate();
			selection_bounds = selection->GetBoundingRect();
		}
	...
	bool first_click)
{
	...
	if (reject_mouse_input == true) {
		if (normPoint.x > 0 && normPoint.y > 0 &&
	...
	if (first_click == TRUE) {
		...
		if (move_all == TRUE) {
			...
			float delta_x = previous_point.x - point.x;
			float delta_y = previous_point.y - point.y;
	...
	, fManipulator(manipulator)
{
	...
	BButton* restoreWidthButton = new BButton(B_TRANSLATE("Restore"),
		new BMessage(SCALE_RESTORE_WIDTH));
	...
	BButton* restoreHeightButton = new BButton(B_TRANSLATE("Restore"),
		new BMessage(SCALE_RESTORE_HEIGHT));
	...
	BCheckBox* proportion_box = new BCheckBox(B_TRANSLATE_COMMENT("Lock",
		"Keep the aspect ratio"), new BMessage(SCALE_PROPORTION_CHANGED));
