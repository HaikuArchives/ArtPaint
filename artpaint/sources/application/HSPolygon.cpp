/*
	...
	Filename:	HSPolygon.cpp
	Contents:	HSPolygon-class definitions.
...
void HSPolygon::AddPoints(BPoint *point_list,int32 num_points,bool reverse_points)
{
...
void HSPolygon::Rotate(const BPoint &center,float angle)
{
...
void HSPolygon::RotateAboutCenter(float angle)
{
	// Do for each point:
...
void HSPolygon::RoundToInteger()
{
	// This rounds every point in the polygon to nearest point that has integer
	// as it's coordinates
	for (int32 i=0;i<point_count;i++) {
		points[i].x = (((points[i].x - floor(points[i].x)) < 0.5)? floor(points[i].x) : ceil(points[i].x));
		points[i].y = (((points[i].y - floor(points[i].y)) < 0.5) ? floor(points[i].y) : ceil(points[i].y));
	}
}

BRect HSPolygon::BoundingBox()
{
...
void HSPolygon::SetMaximumInterPointDistance(float max_dist)
{
...
void HSPolygon::ChangeDirection(polygon_direction dir)
{
...
polygon_direction HSPolygon::GetDirection()
{
	return direction;
}
