...
TransparencyManipulator::TransparencyManipulator(BBitmap *bm)
		: WindowGUIManipulator(), ImageAdapter()
{
	if (bm != NULL) {
		preview_bitmap = bm;
	...
	previous_transparency_change = 0;
...
BBitmap* TransparencyManipulator::ManipulateBitmap(ManipulatorSettings *set,BBitmap *original, Selection *selection,BStatusBar *status_bar)
{
	return NULL;
}
...
int32 TransparencyManipulator::PreviewBitmap(Selection *selection,bool full_quality,BRegion *updated_region)
{
...
void TransparencyManipulator::SetPreviewBitmap(BBitmap *bm)
{
	if (image != NULL) {
...
void TransparencyManipulator::SetTransparency(float change)
{
...
void TransparencyManipulator::Reset(Selection*)
{
	if (image != NULL) {
		if (image->ContainsLayer(preview_layer))
			preview_layer->SetTransparency(original_transparency_coefficient);
	}
}

BView* TransparencyManipulator::MakeConfigurationView(BMessenger *target)
{
...
ManipulatorSettings* TransparencyManipulator::ReturnSettings()
{
...
TransparencyManipulatorView::TransparencyManipulatorView(BRect rect,TransparencyManipulator *manip,BMessenger *t)
	: WindowGUIManipulatorView(rect)
{
	target = new BMessenger(*t);
	manipulator = manip;

	transparency_control = new ControlSlider(BRect(0,0,200,40),"transparency_control",StringServer::ReturnString(TRANSPARENCY_STRING),new BMessage(MOUSE_TRACKING_FINISHED),0,255,B_TRIANGLE_THUMB);
	transparency_control->SetValue(0);
	transparency_control->SetLimitLabels(StringServer::ReturnString(TRANSPARENT_STRING),StringServer::ReturnString(OPAQUE_STRING));
	...
	ResizeTo(transparency_control->Frame().Width()+20,transparency_control->Frame().Height()+20);
...
void TransparencyManipulatorView::AttachedToWindow()
{
...
void TransparencyManipulatorView::ChangeSettings(TransparencyManipulatorSettings *new_set)
{
	settings = *new_set;
