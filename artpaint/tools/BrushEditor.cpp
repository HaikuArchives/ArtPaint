	...
	fBrushInfo(fBrush->GetInfo())
{
	...
	fBrushView = new BrushView(BRect(0, 0, width, height), fBrush);

	BMessage* message = new BMessage(kBrushShapeChanged);
	message->AddInt32("shape", HS_RECTANGULAR_BRUSH);

	fRectangle = new BRadioButton(B_TRANSLATE("Rectangle"), message);

	message = new BMessage(kBrushShapeChanged);
	message->AddInt32("shape", HS_ELLIPTICAL_BRUSH);

	fEllipse = new BRadioButton(B_TRANSLATE("Ellipse"), message);

	fStoreBrush = new BButton(B_TRANSLATE("Store brush"), new BMessage(kBrushStoreRequest));
	...
	fBrushSize = new NumberSliderControl(B_TRANSLATE("Size:"), "0", message, 1, 500, false, true,
		B_NO_BORDER, B_TRIANGLE_THUMB, true);

	message = new BMessage(kBrushRatioChanged);
	message->AddFloat("value", 0);

	fBrushRatio = new FloatSliderControl(B_TRANSLATE("Aspect:"), "0", message, -10., 10., false);
	...
	fBrushAngle = new NumberSliderControl(B_TRANSLATE("Angle:"), "0", message, -90, 90, false);
	...
	fBrushFade = new NumberSliderControl(B_TRANSLATE("Hardness:"), "0", message, 0, 100, false);
