...
ImageProcessingLibrary::gaussian_blur(BBitmap* bitmap, float radius)
{
	...
	for (int32 y = 0; y < height; y++) {
		...
		b_bits++;
		convolve_1d_fixed(
			source_array + kernel_radius, target_array, width, fixed_kernel, kernel_radius);
	...
	*/
	...
	for (int32 i = 0; i < threadCount; i++) {
		...
		blur_thread_array[i] = spawn_thread(
			start_filter_1d_thread_clockwise, "filter_1d_thread", B_NORMAL_PRIORITY, data);
	...
	for (int32 i = 0; i < threadCount; i++) {
		...
		blur_thread_array[i] = spawn_thread(
			start_filter_1d_thread_counterclockwise, "filter_1d_thread", B_NORMAL_PRIORITY, data);
		resume_thread(blur_thread_array[i]);
	}
...
int32
ImageProcessingLibrary::start_filter_1d_thread_clockwise(void* d)
{
...
int32
ImageProcessingLibrary::start_filter_1d_thread_counterclockwise(void* d)
{
	filter_thread_data* data = (filter_thread_data*)d;
...
void
ImageProcessingLibrary::filter_1d_and_rotate_clockwise(int32* s_bits, int32 s_bpr, int32* d_bits,
	int32 d_bpr, int32 left, int32 right, int32 top, int32 bottom, int32* kernel,
	int32 kernel_radius)
{
...
void
ImageProcessingLibrary::filter_1d_and_rotate_counterclockwise(int32* s_bits, int32 s_bpr,
	int32* d_bits, int32 d_bpr, int32 left, int32 right, int32 top, int32 bottom, int32* kernel,
	int32 kernel_radius)
{
	int32 width = right - left + 1;
	uint32* source_array = new uint32[width + kernel_radius * 2];
...
void
ImageProcessingLibrary::convolve_1d(
	uint32* s, uint32* t, int32 length, float* kernel, int32 kernel_radius)
{
	union {
		uint8 bytes[4];
		uint32 word;
...
void
ImageProcessingLibrary::convolve_1d_fixed(
	uint32* s, uint32* t, int32 length, int32* kernel, int32 kernel_radius)
{
	union {
		uint8 bytes[4];
		uint32 word;
...
status_t
ImageProcessingLibrary::grayscale_ahe(BBitmap* bitmap, int32 regionSize)
{
	int32 right, bottom;
	...
	for (int32 i = 0; i < histogram_count_per_row; i++) {
		if (i == 1) {
			...
			calculate_local_mapping_function(
				bitmap, corner_x, corner_y, regionSize, next_histograms[i]);
	...
	for (int32 y = 0; y <= bottom; y++) {
		...
		for (int32 x = 0; x <= right; x++) {
			...
			map_value = left_coeff
					* (top_coeff * prev_histograms[current_bin - 1][hist_index]
						+ bottom_coeff * next_histograms[current_bin - 1][hist_index])
				+ right_coeff
					* (top_coeff * prev_histograms[current_bin][hist_index]
...
status_t
ImageProcessingLibrary::grayscale_clahe(BBitmap* bitmap, int32 regionSize, int32 clipLimit)
{
	int32 right, bottom;
	right = bitmap->Bounds().right;
	bottom = bitmap->Bounds().bottom;
	...
	for (int32 i = 0; i < histogram_count_per_row; i++) {
		if (i == 1) {
			...
			calculate_local_mapping_function_clip(
				bitmap, corner_x, corner_y, regionSize, clipLimit, next_histograms[i]);
			corner_x = corner_x + regionSize;
		}
	}
...
void
ImageProcessingLibrary::calculate_local_mapping_function(
	BBitmap* bitmap, int32 cx, int32 cy, int32 regionSize, uint8* mapFunction)
{
...
void
ImageProcessingLibrary::calculate_local_mapping_function_clip(
	BBitmap* bitmap, int32 cx, int32 cy, int32 regionSize, int32 clipLimit, uint8* mapFunction)
{
	int32 left, right, top, bottom;
	left = cx;
	top = cy;
