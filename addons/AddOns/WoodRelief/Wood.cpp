...
BBitmap*
WoodManipulator::ManipulateBitmap(BBitmap* original, BStatusBar* status_bar)
{
...
const char*
WoodManipulator::ReturnHelpString()
{
...
int32
WoodManipulator::thread_entry(void* data)
{
	int32 thread_number;
...
int32
WoodManipulator::thread_function(int32 thread_number)
{
	...
	if (selection->IsEmpty()) {
		...
		for (float y = top; y <= bottom; ++y) {
			for (float x = left; x <= right; ++x) {
				...
				// 1/256, and generator is initialized with 0.7,8.
				float z
					= .144 * color3.bytes[0]
					+ .597 * color3.bytes[1]
					+ .299 * color3.bytes[2];

				float noise = generator.PerlinNoise3D(
					x * one_per_width, y * one_per_height, z * one_per_depth);
		...
		for (int32 y = top; y <= bottom; ++y) {
			for (int32 x = left; x <= right; ++x) {
				if (selection->ContainsPoint(x, y)) {
					...
					// This creates a woodlike texture when one_per_width is 1/8 and one_per_height
					// is 1/256, and generator is initialized with 0.7,8.
					float z
						= .144 * color3.bytes[0]
						+ .597 * color3.bytes[1]
						+ .299 * color3.bytes[2];
