...
BBitmap* floyd_steinberg_edd_color_mapper(BBitmap* inSource, const rgb_color* inPalette,
	int inPaletteSize);
BBitmap* preserve_solids_fs_color_mapper(BBitmap* inSource, const rgb_color* inPalette,
	int inPaletteSize);
BBitmap* n_candidate_color_mapper(BBitmap *inSource, const rgb_color* inPalette, int inPaletteSize,
	int maxCandidates = 0);
...
inline int32 find_candidate(uint32 bgra_word,const rgb_color* inPalette, int inPaletteSize,
	int candidateNumber, float* outDistance)
{
	struct Candidate {
		int32 index;
		float distance;
