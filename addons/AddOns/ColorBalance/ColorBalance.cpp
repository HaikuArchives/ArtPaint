...
BBitmap*
ColorBalanceManipulator::ManipulateBitmap(
	ManipulatorSettings* set, BBitmap* original, BStatusBar* status_bar)
{
	ColorBalanceManipulatorSettings* new_settings
		= dynamic_cast<ColorBalanceManipulatorSettings*>(set);
	...
	if ((new_settings->red_difference == 0) && (new_settings->green_difference == 0)
		&& (new_settings->blue_difference == 0))
		return NULL;
	...
	if (selection->IsEmpty() == TRUE) {
		for (int32 y = top; y <= bottom; y++) {
			for (int32 x = left; x <= right; x++) {
				...
				color.bytes[0]
					= max_c(min_c(255, color.bytes[0] + new_settings->blue_difference), 0);
				color.bytes[1]
					= max_c(min_c(255, color.bytes[1] + new_settings->green_difference), 0);
				color.bytes[2]
					= max_c(min_c(255, color.bytes[2] + new_settings->red_difference), 0);
...
int32
ColorBalanceManipulator::PreviewBitmap(bool full_quality, BRegion* updated_region)
{
...
void
ColorBalanceManipulator::SetPreviewBitmap(BBitmap* bm)
{
...
const char*
ColorBalanceManipulator::ReturnHelpString()
{
...
const char*
ColorBalanceManipulator::ReturnName()
{
...
BView*
ColorBalanceManipulator::MakeConfigurationView(const BMessenger& target)
{
...
ManipulatorSettings*
ColorBalanceManipulator::ReturnSettings()
{
...
void
ColorBalanceManipulator::ChangeSettings(ManipulatorSettings* s)
{
	ColorBalanceManipulatorSettings* new_settings
		= dynamic_cast<ColorBalanceManipulatorSettings*>(s);
...
ColorBalanceManipulatorView::ColorBalanceManipulatorView(
	BRect rect, ColorBalanceManipulator* manip, const BMessenger& t)
	:
	WindowGUIManipulatorView()
{
	...
	blue_slider = new BSlider("blue_slider", NULL, new BMessage(HS_MANIPULATOR_ADJUSTING_FINISHED),
		-255, 255, B_HORIZONTAL, B_TRIANGLE_THUMB);
	blue_slider->SetLimitLabels(B_TRANSLATE("Less"), B_TRANSLATE("More"));
...
void
ColorBalanceManipulatorView::AttachedToWindow()
{
...
void
ColorBalanceManipulatorView::MessageReceived(BMessage* message)
{
	switch (message->what) {
...
void
ColorBalanceManipulatorView::ChangeSettings(ColorBalanceManipulatorSettings s)
{
	settings = s;
}
