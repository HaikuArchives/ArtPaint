...
void
TwirlManipulator::MouseDown(BPoint point, uint32, BView*, bool first_click)
{
	if (first_click)
...
BBitmap*
TwirlManipulator::ManipulateBitmap(
		...
		for (float y = start_y; y <= end_y; y++) {
			for (float x = 0; x < source_bpr; x++) {
				...
				if (distance <= center_distance_from_edges) {
					float omega = (center_distance_from_edges - distance)
						/ center_distance_from_edges * k * 2 * PI;
			...
			for (float x = 0; x < source_bpr; x++) {
				if (selection->ContainsPoint(x, y)) {
					...
					*target_bits++ = combine_4_pixels(p1, p2, p3, p4,
						(1 - y_mix_upper) * (1 - x_mix_right), (1 - y_mix_upper) * (x_mix_right),
						(y_mix_upper) * (1 - x_mix_right), (y_mix_upper) * (x_mix_right));
				} else
...
int32
TwirlManipulator::PreviewBitmap(bool full_quality, BRegion* updated_region)
{
			...
			for (int32 y = start_y; y <= end_y; y += last_calculated_resolution) {
				for (int32 x = 0; x < source_bpr; x += last_calculated_resolution) {
					...
					if (distance <= center_distance_from_edges) {
						...
						dx = (cos_table[cos_angle] * real_x - sin_table[sin_angle] * real_y)
							- real_x;
						dy = (sin_table[sin_angle] * real_x + cos_table[cos_angle] * real_y)
							- real_y;
					} else {
						dx = 0;
						dy = 0;
					...
					if (((y + dy) <= bottom) && ((y + dy) >= top) && ((x + dx) >= 0)
						&& ((x + dx) < target_bpr)) {
...
void
TwirlManipulator::SetPreviewBitmap(BBitmap* bm)
{
...
BView*
TwirlManipulator::MakeConfigurationView(const BMessenger& target)
{
...
const char*
TwirlManipulator::ReturnHelpString()
{
...
ManipulatorSettings*
TwirlManipulator::ReturnSettings()
{
...
void
TwirlManipulator::ChangeSettings(ManipulatorSettings* s)
{
...
TwirlManipulatorView::TwirlManipulatorView(BRect rect, TwirlManipulator* manip, const BMessenger& t)
	:
	WindowGUIManipulatorView()
{
	...
	twirl_radius_slider = new BSlider("twirl_radius_slider", B_TRANSLATE("Size:"),
		new BMessage(TWIRL_RADIUS_CHANGED), 10, 1000, B_HORIZONTAL, B_TRIANGLE_THUMB);
	...
	twirl_amount_slider = new BSlider("twirl_amount_slider", B_TRANSLATE("Direction and amount:"),
		new BMessage(TWIRL_AMOUNT_CHANGED), MIN_TWIRL_AMOUNT, MAX_TWIRL_AMOUNT, B_HORIZONTAL,
		B_TRIANGLE_THUMB);
...
void
TwirlManipulatorView::AttachedToWindow()
{
...
void
TwirlManipulatorView::AllAttached()
{
...
void
TwirlManipulatorView::MessageReceived(BMessage* message)
{
	switch (message->what) {
...
void
TwirlManipulatorView::ChangeSettings(TwirlManipulatorSettings* s)
{
	settings = *s;
