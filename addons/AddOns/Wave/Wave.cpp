...
instantiate_add_on(BBitmap* bm, ManipulatorInformer* i)
{
	...
	// from WindowGuiManipulator base-class. It will be deleted
...
BBitmap*
WaveManipulator::ManipulateBitmap(
	ManipulatorSettings* set, BBitmap* original, BStatusBar* status_bar)
{
	...
	float R = sqrt(
		pow(max_c(fabs(cx), target_bpr - fabs(cx)), 2) + pow(max_c(fabs(cy), end_y - fabs(cy)), 2));
			...
			for (float x = 0; x < float_target_bpr; x++) {
				...
				if ((real_x != 0) && (real_y != 0)) {
					...
					dx = real_x / sqrt_x_plus_y * A
						* sin(sqrt_x_plus_y * two_pi_per_s); // The sin and div are slow.
					...
					if ((real_x != 0) && (real_y != 0)) {
						...
						if ((ceil(y + dy) <= bottom) && (ceil(y + dy) >= top)) {
							if ((floor(x + dx) >= 0) && (floor(x + dx) < source_bpr)) {
								p1 = *(source_bits + (int32)ceil(y + dy) * source_bpr
									+ (int32)floor(x + dx));
							} else
								p1 = background.word;

							if ((ceil(x + dx) >= 0) && (ceil(x + dx) < source_bpr)) {
								p2 = *(source_bits + (int32)ceil(y + dy) * source_bpr
									+ (int32)ceil(x + dx));
							} else
								p2 = background.word;
						} else {
						...
						if ((floor(y + dy) <= bottom) && (floor(y + dy) >= top)) {
							if ((floor(x + dx) >= 0) && (floor(x + dx) < source_bpr)) {
								p3 = *(source_bits + (int32)floor(y + dy) * source_bpr
									+ (int32)floor(x + dx));
							} else
								p3 = background.word;

							if ((ceil(x + dx) >= 0) && (ceil(x + dx) < source_bpr)) {
								p4 = *(source_bits + (int32)floor(y + dy) * source_bpr
									+ (int32)ceil(x + dx));
							} else
								p4 = background.word;
						} else {
						...
						*target_bits++ = combine_4_pixels(p1, p2, p3, p4,
							(1 - y_mix_upper) * (1 - x_mix_right),
							(1 - y_mix_upper) * (x_mix_right), (y_mix_upper) * (1 - x_mix_right),
							(y_mix_upper) * (x_mix_right));
					} else {
						if (real_x != 0) {
							sqrt_x_plus_y = sqrt(real_x * real_x);
							dx = real_x / sqrt_x_plus_y * A * sin(sqrt_x_plus_y * two_pi_per_s);
							...
							if ((floor(x + dx) >= 0) && (floor(x + dx) < target_bpr)) {
								p2 = *(
									source_bits + (int32)y * target_bpr + (int32)floor((x + dx)));
							} else
								p2 = background.word;
							*target_bits++ = mix_2_pixels_fixed(p1, p2, 32768 * x_mix_right);
						} else if (real_y != 0) {
							...
							if ((ceil(y + dy) <= bottom) && (ceil(y + dy) >= top)) {
								p1 = *(
									source_bits + (int32)x + (int32)ceil((y + dy)) * target_bpr);
							} else
								p1 = background.word;

							if ((floor(y + dy) <= bottom) && (floor(y + dy) >= top)) {
								p2 = *(
									source_bits + (int32)x + (int32)floor((y + dy)) * target_bpr);
							} else
								p2 = background.word;
							*target_bits++ = mix_2_pixels_fixed(p2, p1, 32768 * y_mix_upper);
						} else
...
WaveManipulator::PreviewBitmap(bool full_quality, BRegion* updated_region)
{
	...
	if (last_calculated_resolution > 0) {
		...
		float k = 0;
		float R = sqrt(pow(max_c(fabs(cx), target_bpr - fabs(cx)), 2)
			+ pow(max_c(fabs(cy), end_y - fabs(cy)), 2));
			...
			for (float y = start_y; y <= end_y; y += last_calculated_resolution) {
				...
				for (float x = 0; x < source_bpr; x += last_calculated_resolution) {
					...
					if ((real_x != 0) && (real_y != 0)) {
						...
						// On intel this does not yet work and the corresponding function is very
						// slow.
#ifdef __POWERPC__
						one_per_sqrt_x_plus_y
							= reciprocal_of_square_root(real_x * real_x + real_y * real_y);
...
//						sqrt_x_plus_y = fsqrt(real_x*real_x+real_y*real_y);
...
#endif
						dx = one_per_sqrt_x_plus_y * A
							* sin_table[(int32)(sqrt_x_plus_y * two_360_per_s) % 720];
						dx = dx * (1 - k * sqrt_x_plus_y / R);
						dy = dx * real_y;
						dx = dx * real_x;
						// This if is quite slow.
						if (((y + dy) <= end_y) && ((y + dy) >= 0) && ((x + dx) >= 0)
							&& ((x + dx) < target_bpr)) {
							*(target_bits + (int32)x + y_target_bpr)
								= *(source_bits + (int32)(x + dx) + (int32)(y + dy) * source_bpr);
						} else
						...
						if (real_x != 0) {
							...
							dx = one_per_sqrt_x_plus_y * A
								* sin_table[(int32)(sqrt_x_plus_y * two_360_per_s) % 720] * real_x;
							dx = dx * (1 - k * sqrt_x_plus_y / R);
							if (((x + dx) >= 0) && ((x + dx) < target_bpr)) {
								*(target_bits + (int32)x + y_target_bpr)
									= *(source_bits + (int32)y * target_bpr + (int32)(x + dx));
							} else
								*(target_bits + (int32)x + y_target_bpr) = background.word;
						} else if (real_y != 0) {
							one_per_sqrt_x_plus_y = reciprocal_of_square_root(real_y * real_y);
							sqrt_x_plus_y = 1 / one_per_sqrt_x_plus_y;
							dy = one_per_sqrt_x_plus_y * A
								* sin_table[(int32)(sqrt_x_plus_y * two_360_per_s) % 720] * real_y;
							dy = dy * (1 - k * sqrt_x_plus_y / R);
							if (((y + dy) <= end_y) && ((y + dy) >= 0)) {
								*(target_bits + (int32)x + y_target_bpr)
									= *(source_bits + (int32)x + (int32)(y + dy) * source_bpr);
							} else
								*(target_bits + (int32)x + y_target_bpr) = background.word;
						} else {
							*(target_bits + (int32)x + y_target_bpr)
...
void
WaveManipulator::MouseDown(BPoint point, uint32, BView*, bool first_click)
{
	if (first_click == TRUE)
		previous_settings = settings;
...
// int32 WaveManipulator::wave_func(uint32 *target_bits,uint32 *source_bits,int32 target_bpr,int32
// source_bpr,int32 start_y,int32 end_y,int32 amount_of_wave,int32 length_of_wave,int32
// dampening_of_wave,float center_x, float center_y,float top_of_image,float bottom_of_image)
//{
...
//	cy = floor(center_y);
//	float R =
//sqrt(pow(max_c(abs(cx),target_bpr-abs(cx)),2)+pow(max_c(abs(cy),bottom_of_image-abs(cy)),2));
//
//
//	float real_x,real_y;
//	float two_pi_per_s = 2*PI/s;
...
//				if ((real_x != 0) && (real_y != 0)) {
//					sqrt_x_plus_y = sqrt(real_x*real_x+real_y*real_y);
//					dx = real_x/sqrt_x_plus_y*A*sin(sqrt_x_plus_y*two_pi_per_s);	// The sin and div are
//slow. 					dx = dx*(1-k*sqrt_x_plus_y/R); 					dy = dx/real_x*real_y; 					float y_mix_upper = ceil(y+dy)-(y+dy);
//					float x_mix_right = (x+dx)- floor(x+dx);
//					if ((ceil(y+dy) <= bottom) && (ceil(y+dy)>=top)) {
//						if ((floor(x+dx) >= 0) && (floor(x+dx) <source_bpr)) {
...
//					}
//					*target_bits++ =
//combine_4_pixels(p1,p2,p3,p4,(1-y_mix_upper)*(1-x_mix_right),(1-y_mix_upper)*(x_mix_right),(y_mix_upper)*(1-x_mix_right),(y_mix_upper)*(x_mix_right));
//				}
//				else {
//					if (real_x != 0) {
//						sqrt_x_plus_y = sqrt(real_x*real_x);
...
//
//					dx =
//one_per_sqrt_x_plus_y*A*sin_table[(int32)(sqrt_x_plus_y*two_360_per_s)%720]; 					dx =
//dx*(1-k*sqrt_x_plus_y/R); 					dy = dx*real_y; 					dx = dx*real_x;
//					// This if is quite slow.
...
//						sqrt_x_plus_y = 1/one_per_sqrt_x_plus_y;
//						dx =
//one_per_sqrt_x_plus_y*A*sin_table[(int32)(sqrt_x_plus_y*two_360_per_s)%720]*real_x; 						dx =
//dx*(1-k*sqrt_x_plus_y/R); 						if (((x+dx) >= 0) && ((x+dx)<target_bpr)) 							*target_bits = *(source_bits +
//(int32)y*target_bpr + (int32)(x +dx)); 						else 							*target_bits = 0xFFFFFF00;
//					}
//					else if (real_y != 0) {
//						one_per_sqrt_x_plus_y = reciprocal_of_square_root(real_y*real_y);
//						sqrt_x_plus_y = 1/one_per_sqrt_x_plus_y;
//						dy =
//one_per_sqrt_x_plus_y*A*sin_table[(int32)(sqrt_x_plus_y*two_360_per_s)%720]*real_y; 						dy =
//dy*(1-k*sqrt_x_plus_y/R); 						if (((y+dy)<=bottom) && ((y+dy) >= top)) 							*target_bits = *(source_bits +
//(int32)x + (int32)(y + dy)*target_bpr); 						else 							*target_bits = 0xFFFFFF00;
//					}
//					else {
...
//						dx = dx*(1-k*real_x/R);
//						if (((x+dx) >= 0) && ((x+dx)<target_bpr)) {
//							uint32 new_value = *(source_bits + (int32)y*target_bpr + (int32)(x
//+dx)); 							*target_bits = new_value;
//							*(target_bits+1) = new_value;
//							*(target_bits+target_bpr) = new_value;
//							*(target_bits+target_bpr+1) = new_value;
//						}
...
//							int32 new_value = *(source_bits + (int32)x + (int32)(y +
//dy)*target_bpr); 							*target_bits = new_value;
//							*(target_bits+1) = new_value;
//							*(target_bits+target_bpr) = new_value;
//							*(target_bits+target_bpr+1) = new_value;
//						}
...
const char*
WaveManipulator::ReturnHelpString()
{
...
void
WaveManipulator::SetPreviewBitmap(BBitmap* bm)
{
...
BView*
WaveManipulator::MakeConfigurationView(const BMessenger& target)
{
...
ManipulatorSettings*
WaveManipulator::ReturnSettings()
{
...
void
WaveManipulator::ChangeSettings(ManipulatorSettings* s)
{
...
WaveManipulatorView::WaveManipulatorView(BRect rect, WaveManipulator* manip, const BMessenger& t)
	:
	WindowGUIManipulatorView()
{
	...
	wave_length_slider = new BSlider("wave_length_slider", B_TRANSLATE("Wavelength:"),
		new BMessage(WAVE_LENGTH_CHANGED), MIN_WAVE_LENGTH, MAX_WAVE_LENGTH, B_HORIZONTAL,
		B_TRIANGLE_THUMB);
	...
	wave_amount_slider = new BSlider("wave_amount_slider", B_TRANSLATE("Strength:"),
		new BMessage(WAVE_AMOUNT_CHANGED), MIN_WAVE_AMOUNT, MAX_WAVE_AMOUNT, B_HORIZONTAL,
		B_TRIANGLE_THUMB);
...
void
WaveManipulatorView::AttachedToWindow()
{
...
void
WaveManipulatorView::AllAttached()
{
...
void
WaveManipulatorView::MessageReceived(BMessage* message)
{
	switch (message->what) {
...
void
WaveManipulatorView::ChangeSettings(WaveManipulatorSettings* s)
{
	settings = *s;
