...
extern "C" __declspec(dllexport) char name[255] = "Detects Edges";
extern "C" __declspec(dllexport) char menu_help_string[255] = "Detects the edges in the image.";
extern "C" __declspec(dllexport) int32 add_on_api_version = ADD_ON_API_VERSION;
extern "C" __declspec(dllexport) add_on_types add_on_type = SHARPEN_FILTER_ADD_ON;
...
BBitmap* DetectEdgesManipulator::ManipulateBitmap(BBitmap *original,Selection *selection,BStatusBar *status_bar)
{
	/*
		...
		to the original. Edges will be detected by using convolution with the following kernel.
