...
//         Texture[tp] = (uint8)(255.0*sin(angle*(double)j)*cos(angle*(double)i));
   ...
   // screenPointer[j+(y*800)] = 0x5f;
	...
	uint8 *render(float **hm,uint8 *t,Surface *s,LightSource **ll,int32 lightCount,int32 X,int32 Y) {
...
//        float lfx,lfy,lfz;          // light focus
        ...
        for(int y=0;y<Y-1;++y) {
            ...
            for(int lightNumber=0;lightNumber<lightCount;++lightNumber) {
                ...
                length = 1.0 / sqrt( (ldx*ldx)+ (ldy*ldy) + (ldz*ldz) );
                ...
                for(int scanx=0;scanx<X-1;++scanx) {
                    ...
                    ly = lpy -hm[y][scanx];
                    lz = lpz -(float)y;
                    length = 1.0 / sqrt( (lx*lx)+ (ly*ly) + (lz*lz) );
                    ...
                    hy = ly + eyey;
                    hz = lz + eyez;
                    length = 1.0 / sqrt( (hx*hx)+ (hy*hy) + (hz*hz) );
